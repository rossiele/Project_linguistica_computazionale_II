...
def operation(file1, file2, op, unify):
	lines1 = [x.split() for x in open(file1, "r", encoding='utf-8')]
	lines2 = [x.split() for x in open(file2, "r", encoding='utf-8')]
	...
	for i in range(len(lines1)):
		...
		if len(splitted_line1) == max:
			...
			if (a!=b):
				count+=1
				print("Differences found at line " + str(i+1) + ", token no. " + splitted_line1[0] + ", token " + splitted_line1[1])
...
#			if op=="dep":
#				if unify and ((splitted_line1[target] != splitted_line2[target]) or (splitted_line1[target+1] != splitted_line2[target+1])):
#					print("Differences found at line " + str(i+1))
	print("Tot diff: ", count)
	out_file.close()
	annotation(output)

def annotation(output):
	t = AnnotationTask(data=[x.split() for x in open(output, "r", encoding='utf-8')])
